...
class _PatientHomeState extends State<PatientHome> {
  ...
  List getAllDates(startdate, enddate, time, interval) {
    ...
    DateTime tempdate = new DateTime(startdate.year, startdate.month,
        startdate.day, daytime.hour, daytime.minute, 0, 0, 0);
    while (tempdate.isBefore(enddate)) {
      dates.add(tempdate);
      tempdate = new DateTime(tempdate.year, tempdate.month,
          tempdate.day + interval, daytime.hour, daytime.minute, 0, 0, 0);
    }
    return dates;
  }
                ...
                builder:
                  ...
                  if (snapshot.connectionState == ConnectionState.waiting) {
                    ...
                    _firebaseMessaging.getToken().then((String token) {
                      print(token);
                      user.email = appState?.firebaseUserAuth?.email;
                      user.userId = appState?.firebaseUserAuth?.uid;
                              ...
                              builder: (context,
                                ...
                                if (snapshot.connectionState ==
                                  ...
                                  return Builder(
                                    ...
                                    List reminderList =
                                        (snapshot?.data ?? Map())["reminders"]
                                                ?.toList() ??
                                            [];
                                    if (reminderList.length > 0) {
                                      List<Widget> tiles = reminderList
                                          .sublist(
                                              0, min(3, reminderList.length))
                                        ...
                                        var time = el["time"];
                                        var timesText =
                                            " at " + timeToString(time);

                                        DateTime startdate =
                                            el["startDateTime"].toDate();
                                        DateTime enddate =
                                            el["endDateTime"].toDate();

                                        String start = dtToString(startdate);
                                        String end = dtToString(enddate);

                                        DateTime td = new DateTime.now();
                                        DateTime today = DateTime(td.year,
                                            td.month, td.day, 23, 59, 59);

                                        List alldates = getAllDates(
                                            startdate, enddate, time, interval);
                                        bool cutoff = alldates.length > 7;
                                        if (cutoff == true) {
                                          List bdates = alldates
                                              .where((x) => today.isAfter(x))
                                              .toList();
                                          List adates = alldates
                                              .where((x) => today.isBefore(x))
                                              .toList();
                                          alldates = List();
                                          bool flipper = true;
                                        ...
                                        List<Widget> dateitems =
                                            alldates.expand((dateel) {
                                          bool isfuture =
                                              today.isBefore(dateel);
                                          bool checked = !isfuture &&
                                              el["checklist"].contains(
                                                                          ...
                                                                          child: Checkbox(
                                                                              ...
                                                                              onChanged: (val) {
                                                                                if (val) {
                                                                                  ...
                                                                                  Firestore.instance.collection('reminders').document(appState?.firebaseUserAuth?.uid ?? "").updateData({
